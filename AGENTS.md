# Codex Project Instructions — Autonomous Trading Bot (Indian Markets / Upstox)

---

## MANDATORY: CONSULT MASTER.md BEFORE EVERY CHANGE

`MASTER.md` at repo root is the **single source of truth** for cross-module wiring, strategy catalog, DB schema, scheduler jobs, and a per-function index. Regenerated by `python3 scripts/gen_master_doc.py`.

**Workflow for ANY change (new feature, bug fix, refactor, config tweak):**

1. **Read** the relevant sections of MASTER.md first (wiring matrix, strategy catalog, dead-code list, scheduler table). Each session is headless — MASTER.md is the persistent memory across sessions.
2. **Report status to the user before touching code**:
   - Which MASTER.md sections the change affects (wiring / strategy / schedulers / DB / dead-code).
   - What modules / functions / configs will be touched.
   - Whether new wiring is needed (param to pass, callback to register).
   - Whether the change risks orphaning a param or duplicating a path already documented as dead.
   - Estimated effect: positive, negative, or neutral on each touched area.
3. **Wait for the user's go/no-go** before editing code.
4. **Make the change.** Add new dead-params to the "Known dead code" section if any wiring is left incomplete.
5. **Regenerate MASTER.md** in the same commit: `python3 scripts/gen_master_doc.py`.
6. **Commit message** must reference which MASTER.md section was affected.

This rule overrides default behaviour. Even small changes (config knob tweak, one-line fix) require step 2 — a one-paragraph status update. If MASTER.md is missing or stale, regenerate it FIRST before evaluating the change.
